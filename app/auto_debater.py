...
class AIObserver:
    """WebSocket observer for AI-vs-AI debates.

    Connects to the WebSocket and watches events without sending any actions.
    Both sides are generated by the AI agent — no human input needed.

    Usage::

        observer = AIObserver(base_url="http://localhost:8000", debate_id="abc123")
    ...
    async def run(self) -> Dict[str, Any]:
        """Connect and observe until judge_result or timeout."""
        logger.info("AIObserver connecting to %s", self.ws_url)
        ...
        try:
            async with websockets.connect(self.ws_url) as ws:
                ...
                try:
                    ...
                    logger.error("AIObserver timed out after %ss", self.timeout)
                finally:
                    receive_task.cancel()
                    try:
                        await receive_task
            ...
            logger.error("AIObserver connection error: %s", e)
            return {
                "success": False,
                "error": str(e),
                "events": self.events,
    ...
    async def _receive_loop(self, ws) -> None:
        """Listen for events — no actions sent."""
        async for raw in ws:
            try:
                data = json.loads(raw)
            except json.JSONDecodeError:
            ...
            self.events.append(data)
            event_type = data.get("type", "")
            logger.info("AIObserver received: %s", event_type)

            if event_type == "judge_result":
                self.judge_result = data
                self._completed.set()
            elif event_type == "disconnect":
                logger.info("AI-AI debate disconnected: %s", data.get("reason"))
                self._completed.set()
